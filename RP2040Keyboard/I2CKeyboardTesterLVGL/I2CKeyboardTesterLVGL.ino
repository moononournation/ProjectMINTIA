...
void setup()
{
  ...
  // while(!Serial);
  Serial.println("I2C Keyboard Tester");
  ...
  else
  {
    ...
    /* Initialize I2C keyboard input device driver */
    lv_indev_t * indev_key = lv_indev_create();
    lv_indev_set_type(indev_key, LV_INDEV_TYPE_KEYPAD);
    lv_indev_set_read_cb(indev_key, my_keyboard_read);
    lv_group_t * g = lv_group_create();
    lv_group_set_default(g);
    lv_indev_set_group(indev_key, g);

    // simple example
    lv_obj_t *obj = lv_textarea_create(lv_scr_act());
    lv_obj_set_size(obj, screenWidth, screenHeight);
    lv_obj_align(obj, LV_ALIGN_CENTER, 0, 0);
    lv_textarea_set_placeholder_text(obj, "I2C Keyboard Tester");
